...
export QQBC_INSTALL_DIR=${INSTALL_LOCATION:-$QQBC_INSTALL_DIR}

if [[ ! -d "${QQBC_INSTALL_DIR}" ]]; then
   echo "[QQBC installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${QQBC_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/QQBC/eos/issues/6940
   echo "[QQBC installation found: ${QQBC_INSTALL_DIR}]" && INSTALL_PATHS+=("${QQBC_INSTALL_DIR}") # QQBC_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${QQBC_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            echo " - QQBC Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled QQBC Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi
